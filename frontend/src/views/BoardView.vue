  ...
  <div class="board-layout">
    ...
    <aside class="sidebar" :class="{ open: sidebarOpen }">
      ...
      <nav class="sidebar-nav">
        ...
        <a class="nav-item nav-add-board" @click="showCreateBoard = true"><span>＋</span> Новая доска</a>
      </nav>
    ...
    <main class="board-main">
      <header class="board-header">
        ...
        <button class="btn btn-primary" @click="showTaskModal = true">+ Новая задача</button>
      </header>
      ...
      <div v-else class="kanban-board">
        <div class="kanban-lane">
          ...
          >
            <template #item="{ element }">
              <TaskCard :task="element" :stages="stages" @click="openTask(element.id)" @delete="deleteTask" />
            </template>
            <template #footer>
              <div v-if="tasks.filter(t => !t.stageId).length === 0 && !isDragging" class="lane-empty">
        ...
        <div v-for="stage in stages" :key="stage.id" class="kanban-lane" :style="`--lane-color: ${stage.color}`">
          <div class="lane-header">
            <div class="lane-title-row">
          ...
          >
            ...
            <template #footer>
              <div v-if="tasks.filter(t => t.stageId === stage.id).length === 0 && !isDragging" class="lane-empty">
                Перетащите задачу сюда
              </div>
            </template>
          </draggable>
    ...
    <TaskModal v-if="showTaskModal" :stages="stages" @close="showTaskModal = false" @created="onTaskCreated" />
    <StagesModal v-if="showStagesModal" :stages="stages" @close="showStagesModal = false" @updated="stagesStore.fetch()" />
...
.nav-badge { margin-left: auto; background: var(--danger); color: #fff; font-size: 11px; font-weight: 700; padding: 1px 7px; border-radius: 99px; min-width: 18px; text-align: center; }
.sidebar-bottom { margin-top: auto; }
.stats-card { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; padding: 16px; display: flex; align-items: center; justify-content: space-around; }
...
.user-badge { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; margin-top: 8px; background: var(--surface2); border-radius: 9px; border: 1px solid var(--border); }
...
.hamburger span { display: block; width: 22px; height: 2px; background: var(--text); border-radius: 99px; transition: all .2s; }
...
.board-header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 36px; }
...
.kanban-lane { background: var(--surface); border: 1px solid var(--border); border-radius: 16px; overflow: hidden; transition: border-color .2s; }
...
.lane-count { background: var(--surface2); border: 1px solid var(--border); border-radius: 99px; font-size: 11px; padding: 1px 8px; color: var(--muted); font-family: 'DM Sans', sans-serif; }
...
.lane-empty { grid-column: 1 / -1; text-align: center; padding: 24px; color: var(--muted); font-size: 13px; border: 1px dashed var(--border); border-radius: 10px; transition: border-color .2s; }
...
@media (max-width: 768px) {
  ...
  .sidebar { position: fixed; left: -260px; top: 0; height: 100vh; width: 240px; transition: left .25s ease; box-shadow: none; }
